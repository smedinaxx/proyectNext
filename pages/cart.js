...
export default function Cart() {
    ...
    return (
        <div className="bg-neutral-900 pt-32 pb-8 px-4">
            <h1 className="text-white text-2xl md:text-3xl lg:text-4xl xl:text-7xl pb-8 flex justify-center">Shopping Summary</h1>
            <LayoutSimple>
                {cart.length === 0 ? (
                    <p className="text-lime-700 pb-4">
                    Your cart is empty. 
                    <span className="mx-8"> {/* Ajusta el margen según tus necesidades */}
                    ...
                    <>
                            ...
                            {cart.map(product => (
                                <li key={product.id} className="flex items-center border-b border-gray-600 py-4">
                                    ...
                                    <div className="w-1/2 pl-4">
                                        ...
                                        <p className="text-md md:text-xl lg:text-2xl xl:text-3xl text-amber-200">Subtotal: {formatCurrency(product.price * product.quantity)}</p> {/* Mostrar el total del producto */}
                                        <button
                        ...
                        <div className="flex justify-center mt-4">
                            <Link href="/checkout">
                                <button className="bg-blue-500 text-white py-2 px-6 rounded mb-8">
                                    Checkout
                                </button>
                            </Link>
                            <button
